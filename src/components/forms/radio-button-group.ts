...
const RADIO_BUTTON_GROUP_VALUE_ACCESSOR = {
...
export class RadioButtonGroup implements ControlValueAccessor {
    ...
    private _updateRadios() {
        this._radios.forEach((radio) => radio.markChecked(this._value));
    }
}
...
@Directive({
    ...
    host: {'(change)': 'onChange($event.target.value)', '[checked]': 'isChecked'}
})
export class NgbRadio implements OnDestroy {
    ...
    constructor(@Optional() private _group: RadioButtonGroup,
                @Optional() private _label: NgbActiveLabel,
...
//export const NGB_RADIO_DIRECTIVES = [NgbRadio, NgbActiveLabel, RadioButtonGroup];