...
task(':publish:whoami', execTask('npm', [
    'whoami',
    '-registry',
], {
    silent: true,
...
function _execNpmPublish(label: string): Promise<{}> {
    ...
    console.log('Setting package director to:', packageDir);
    process.chdir(packageDir);
    console.log(`Publishing Synergetic core...`);
    ...
    return new Promise((resolve, reject) => {
        ...
        childProcess.on('close', (code: number) => {
            if (code == 0) {
                ...
                reject(new Error(`Unable to publish, status: ${code}.`));
            }
        });
    });
}
